...
export default function AcceptInvitationPage() {
  ...
  const { register, handleSubmit, watch } = useForm<AcceptForm>();
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-3xl shadow-xl p-8 text-center">
            ...
            <p className="text-gray-500 mb-6">{tokenError || 'Cette invitation est invalide ou a expiré.'}</p>
            <Link
              to="/login"
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-3xl shadow-xl p-8 text-center">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">Bienvenue chez {invitationInfo?.merchant_name} !</h2>
            <p className="text-gray-500 text-sm mb-6">
              Votre compte a été activé. Redirection vers votre espace...
            </p>
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-3xl shadow-xl p-8">
          ...
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Rejoignez l'équipe !</h2>
            <p className="text-gray-500 text-sm">
              Vous êtes invité(e) à rejoindre <strong>{invitationInfo?.merchant_name}</strong>
            </p>
          </div>

          {/* Invitation Details */}
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-100 rounded-xl p-4 mb-6 space-y-3">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm">
                <span className="text-lg font-bold text-green-600">
                  {invitationInfo?.first_name?.[0]}{invitationInfo?.last_name?.[0]}
                </span>
              </div>
              <div>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-green-100">
              <span className={`px-3 py-1 text-xs font-medium rounded-full ${
                invitationInfo?.role === 'MERCHANT' 
                  ? 'bg-purple-100 text-purple-700' 
                  : 'bg-blue-100 text-blue-700'
              }`}>
                {invitationInfo?.role_display}
              </span>
              {invitationInfo?.outlet_name && (
            ...
            {invitationInfo?.invited_by && (
              <p className="text-xs text-gray-500 pt-2 border-t border-green-100">
                Invité par <strong>{invitationInfo.invited_by}</strong>
              </p>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <button
              ...
              className="w-full py-3 px-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-semibold rounded-xl hover:from-green-600 hover:to-emerald-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
